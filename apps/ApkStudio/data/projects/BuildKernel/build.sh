#!/bin/bash
# ApkStudio Build Script for BuildKernel

APP_NAME="BuildKernel"
PKG_NAME="com.conjure.buildkernel"
PROJ_DIR=$(pwd)
...
# Extract and bundle all required Termux shared libraries recursively
gather_libs() {
    local target_bin=$1
    for lib in $(readelf -d "$target_bin" 2>/dev/null | grep NEEDED | awk -F'[' '{print $2}' | awk -F']' '{print $1}'); do
        # Only copy libraries that belong to Termux (ignore Android system libs like libc.so)
        if [ -f "$PREFIX/lib/$lib" ] && [ ! -f "lib/$ABI/$lib" ]; then
            cp "$PREFIX/lib/$lib" "lib/$ABI/"
            # Recurse to find nested dependencies
...
# 2. Patch ELF headers to set internal SONAME and replace internal NEEDED versioned names with clean .so names
for file in *; do
    if [ -f "$file" ]; then
        # Set internal SONAME to match the unversioned file name (essential for Android linker's verneed checks)
...
gather_dex() {
    ...
    # 1. Check if Termux provides pre-compiled dexed jars
    if [ -f "$PREFIX/share/dex/$JAR_NAME" ]; then
        cp "$PREFIX/share/dex/$JAR_NAME" "assets/$OUT_DEX"
        
    # 2. Check if we have a locally cached version of this dexed tool
    elif [ -f "$CACHE_DIR/$OUT_DEX" ]; then
        echo "Using cached dex for $JAR_NAME..."
        cp "$CACHE_DIR/$OUT_DEX" "assets/$OUT_DEX"
        
    # 3. Fallback: Compile standard Java jar on the fly and cache it
    elif [ -f "$PREFIX/share/java/$JAR_NAME" ]; then
        echo "Dexing $JAR_NAME (this might take a moment, but will be cached)..."
        d8 --release --min-api 24 --lib "$ANDROID_JAR" --output . "$PREFIX/share/java/$JAR_NAME"
        zip -q "$CACHE_DIR/$OUT_DEX" classes*.dex
        rm classes*.dex
        cp "$CACHE_DIR/$OUT_DEX" "assets/$OUT_DEX"
        
    # 4. Fallback for d8 -> r8 wrapper names
        ...
        echo "Using cached dex for r8.jar..."
        cp "$CACHE_DIR/r8_d8.dex" "assets/$OUT_DEX"
    elif [ "$JAR_NAME" = "d8.jar" ] && [ -f "$PREFIX/share/java/r8.jar" ]; then
        echo "Dexing r8.jar (this might take a moment, but will be cached)..."
